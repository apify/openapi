    ...
    for method, operation in path_yaml.items():
        ...
        for legacy_item in all_legacy_link:
            if summary in legacy_item.get('text'):
                # Do something with the matching item
                # For example, you can access its values using item['key']
                # Replace the placeholder code below with your desired logic
        ...
        # if not found_refferece:
        #     notfound += 1
        #     print(f"Could not find matching item for {summary_kabab_case}")
        #     continue
...
print(f'Updated path: {path_path}')
